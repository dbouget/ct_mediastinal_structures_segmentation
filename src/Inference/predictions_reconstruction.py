...
def reconstruct_post_predictions(predictions, parameters, crop_bbox, nib_volume, resampled_volume):
    ...
    probability_thresholds = parameters.predictions_probability_thresholds
...
def perform_ensemble(input_filename, output_path, model_list, user_runtime):
    input_ni = load_nifti_volume(input_filename)
    ensemble_pred = np.zeros(shape=(input_ni.shape))
    for i, model in enumerate(model_list):
        pred_filename = output_path + model + '-pred_LymphNodes.nii.gz'
        predictions_ni = load_nifti_volume(pred_filename)
        if i == 0:
            ensemble_pred = predictions_ni.get_data()[:]
        else:
            ensemble_pred = np.maximum(ensemble_pred, predictions_ni.get_data()[:])

    if user_runtime.predictions_reconstruction_method == 'thresholding':
        ensemble_pred_bin = np.zeros(ensemble_pred.shape).astype('uint8')
        ensemble_pred_bin[ensemble_pred>=user_runtime.predictions_probability_thresholds[0]] = 1
        nib.save(nib.Nifti1Image(ensemble_pred_bin, input_ni.affine), output_path + '-labels_Ensemble_LymphNodes.nii.gz')
    else:
        nib.save(nib.Nifti1Image(ensemble_pred, input_ni.affine), output_path + '-pred_Ensemble_LymphNodes.nii.gz')
